function getApiKeyIfEnabled() {
    return new Promise((resolve) => {
        ...
        browserAPI.storage.sync.get(["groqApiKey", "isEnabled"], (data) => {
...
async function initExtension() {
    ...
    if (website === 'linkedin') {
        cringeGuardExistingLinkedInPosts();
...
function updateCringeStats(postText) {
    const browserAPI = typeof browser !== 'undefined' ? browser : chrome;
    browserAPI.storage.sync.get(["cringeCount", "timeSavedInMinutes"], (data) => {
        ...
        browserAPI.storage.sync.set({ cringeCount: newCount, timeSavedInMinutes: newTimeSavedInMinutes });
    });
}
...
function cringeGuardThisPost(post, filterMode) {
    ...
    if (website === 'linkedin') {
        parentDiv = post.closest('.feed-shared-update-v2__control-menu-container');
    }
    ...
    if (parentDiv) {
        ...
        if (filterMode === 'remove') { // TODO - refactor is needed here.
            ...
            if (website === 'linkedin') {
                postContainer = parentDiv.closest('.feed-shared-update-v2');
            }
...
async function checkForCringe({ actorName, actorDescription, actorSubDescription, postContent }) {
    ...
    const POST_CRITERIA = LINKEDIN_CRITERIA;
        ...
        const response = await fetch(GROQ_API_URL, {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    ...
                    { role: "user", content: website === 'linkedin' ? 
    ...
    if (commentaryElement && !alreadyProcessedPosts.has(commentaryElement)) {
        ...
        if (isCringe) {
            ...
            const { filterMode } = await new Promise(resolve => {
                browserAPI.storage.sync.get(['filterMode'], data => {
...
function cringeGuardExistingLinkedInPosts() {
